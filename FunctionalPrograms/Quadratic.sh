...
then
root1=$(awk -v del=$delta -v x=$a -v y=$b "BEGIN { print((-y + (det**0.5))/(2*x))}" )
root2=$(awk -v del=$delta -v x=$a -v y=$b "BEGIN { print((-y - (det**0.5))/(2*x))}" )
...
imaginary=$(awk -v del=$delta -v x=$a "BEGIN { print((-del/(2*x))**0.5)}" )
